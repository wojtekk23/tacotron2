...
class TextMelEmbedLoader(torch.utils.data.Dataset):
    """
        1) loads audio, text, speaker embedding tuples
        2) normalizes text and converts them to sequences of one-hot vectors
        3) computes mel-spectrograms from audio files.
        4) calculates speaker embeddings for audio files
    """
    def __init__(self, audiopaths_and_text, hparams):
        self.audiopaths_and_text = load_filepaths_and_text(audiopaths_and_text)
        self.text_cleaners = hparams.text_cleaners
    ...
    def get_mel_text_embed_tuple(self, audiopath_and_text):
        # separate filename and text
        audiopath, text = audiopath_and_text[0], audiopath_and_text[1]
        text = self.get_text(text)
        mel = self.get_mel(audiopath)
...
class TextMelEmbedCollate():
    ...
    def __call__(self, batch):
        ...
        PARAMS
        ------
        batch: [text_normalized, mel_normalized, speaker_embedding]
        """
        # Right zero-pad all one-hot text sequences to max input length
        input_lengths, ids_sorted_decreasing = torch.sort(
            torch.LongTensor([len(x[0]) for x in batch]),
